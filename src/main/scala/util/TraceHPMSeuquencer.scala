...
import freechips.rocketchip.util.{TraceEncoderParams, TraceEncoderControlInterface}
...
class TraceHPMSequencer (val params: TraceEncoderParams) extends Module {
  val io = IO(new Bundle {
    val control = Input(new TraceEncoderControlInterface())
    val time = Input(UInt(params.coreParams.xlen.W))
  ...
  val byte_buffer = Module(new Queue(UInt(8.W), params.bufferDepth * 2))
